...
"the default VIMRUNTIME is /usr/share/vim/vim73, but in colors path, there is no molokai colorscheme, so you should
"install molokai into $VIMRUNTIMR/colors
if has("gui_running")
    colorscheme molokai
...
au BufRead,BufNewFile *.s,*.asm,*.h,*.hpp,*.c,*.cpp,*.cxx,*.cc,*.java,*.cs,*.pl,*.py,*.sh,*.tex,*.v,*.txt,*.log 2match Underlined /.\%81v/
"
...
nnoremap <Left> <nop>
nnoremap <Right> <nop>
"define Ctrl+h/j/k/l to move cursor
nmap <C-h> <C-w>h
nmap <C-j> <C-w>j
nmap <C-k> <C-w>k
nmap <C-l> <C-w>l
